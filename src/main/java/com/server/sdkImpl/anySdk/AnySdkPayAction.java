...
public class AnySdkPayAction implements ISdkPayAction {
	...
	boolean checkRet = anySdkService.checkGProductId(productId, amount);
	if (!checkRet) {
	    logger.error("[订单异常][产品id,或金额异常]orderId:" + orderId + "|productId:" + productId + "|amount" + amount2);
	...
	try {
	    ...
	    int amount2 = (int) (Float.parseFloat(amount) * 100);
	    if (!orderService.checkOrder(order, channelId, productId, amount2, userId, roleId, serverId)) {
		payDes = "[订单异常][产品id,或金额异常]orderId:" + orderId + "|productId:" + productId + "|amount" + amount2;
		order.setPaydes(payDes);
		logger.error(payDes);
		HttpUtil.write(response, "fail");
		return;
